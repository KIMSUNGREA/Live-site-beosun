...
if (!class_exists("rtmember_user"))
{
	class rtmember_user
	{
         ...
         * @return bool|string
         */
        private function getHttp($url, $headers=null)
        {
            $ch = curl_init();
		...
		function login($as_id, $as_pw) {
			...
			//구웹 확인
			$ck = $this->dbo->fetch("SELECT * FROM ".$this->tbl." WHERE user_id='".$as_id."' LIMIT 1");

			//회원 검색
			if ($ck['oldweb_en'] == "y") {
				$encode_pw = $this->dbo->fetch("SELECT PASSWORD('".$as_pw."') AS pass");
				$r = $this->dbo->fetch("SELECT * FROM ".$this->tbl." WHERE user_id='".$as_id."' AND oldweb_pw='".$encode_pw['pass']."' LIMIT 1");
			} else {
				//받은 비밀번호 암호화
			...
			if (empty($r['user_id'])) {
				...
				rt_js_msg("정보를 찾을 수 없습니다. ID/PW를 다시 확인해 주세요.", "parent"); exit;
		...
		function login_sns($as_id, $as_type) {
			...
			//회원 검색
			$r = $this->dbo->fetch("SELECT * FROM ".$this->tbl." WHERE user_id='".$as_id."' AND lgubun='".$as_type."' LIMIT 1");
